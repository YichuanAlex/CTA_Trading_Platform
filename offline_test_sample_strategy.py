...
class OfflineAdapter:
    ...
    def _generate_targets(self, list_bar):
        ...
        try:
            ...
            targets = self.strat.adjust_positions(long_syms, short_syms, list_bar, scores)
        except Exception:
            targets = {}
        try:
            last_int = {str(k): int(round(v)) for k, v in self.last_dict_target_position.items()}
...
def main():
    ...
    api.cwd = ds_base
    try:
        api.dataset_allow_download = True
    except Exception:
        pass
    ...
    # 设定日期区间并登录（用于必要时远端兜底与枚举当日合约）
    try:
        api.set_date_section(begin_day, end_day)
    except Exception:
        pass
    try:
        api.login(str(js.get("user_name", "")), str(js.get("user_passwd", "")))
    except Exception:
        pass

    # 从 dataset 汇总可用交易日；若为空则保留之前设定的区间用于回放与兜底下载
    days_ds = OfflineAdapter.dataset_days(api.cwd, api.data_type, roots, begin_day, end_day)
    if isinstance(days_ds, list) and len(days_ds) > 0:
        api.trading_date_section = days_ds
