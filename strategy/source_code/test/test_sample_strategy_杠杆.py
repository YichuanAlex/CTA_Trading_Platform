...
class test_sample_strategy:
    ...
    def __init__(self) -> None:
        ...
        self.enable_stop_loss: bool = False
    ...
    def on_init(self) -> None:
        ...
        try:
            use_ds = False
            try:
                use_ds = self._dataset_cache_ready_for_period()
            except Exception:
                use_ds = False
            if use_ds:
    ...
    def on_cycle(self, list_bar: List[Any]) -> Tuple[Dict[str, int], bool]:
        ...
        try:
            ...
            pass
        try:
            if not getattr(self, "main_contracts", {}):
        ...
        self.last_rebalance_day = trading_day
        try:
    ...
    def generate_signals(self, main_bars: List[Any], all_bars: List[Any]) -> Tuple[List[str], List[str], Dict[str, float]]:
        ...
        long_candidates = sorted([it for it in items if (score.get(it[0], 0.0) > thr)], key=lambda x: score.get(x[0], 0.0), reverse=True)[: long_pool_size]
        short_candidates = sorted([it for it in items if (score.get(it[0], 0.0) < -thr)], key=lambda x: score.get(x[0], 0.0))[: short_pool_size]
    ...
    def adjust_positions(self, long_syms: List[str], short_syms: List[str], list_bar: List[Any], scores: Dict[str, float]) -> Dict[str, int]:
        ...
        try:
            selected_short_roots = [r for r in selected_short_roots if r not in selected_long_roots]
            if len(selected_short_roots) < int(self.short_count):
                for r in short_rank_roots:
                    if len(selected_short_roots) >= int(self.short_count):
                        break
                    if (r in selected_short_roots) or (r in selected_long_roots):
                        continue
                    selected_short_roots.append(r)
            selected_short_roots = sorted(selected_short_roots, key=lambda x: float(scores_map.get(x, 0.0)))[: int(self.short_count)]
        except Exception:
            pass
        try:
            long_syms = [str(main_map.get(r, "")) for r in selected_long_roots if str(main_map.get(r, ""))]
            short_syms = [str(main_map.get(r, "")) for r in selected_short_roots if str(main_map.get(r, ""))]
        except Exception:
            pass
        ...
        capital_total = max(0.0, float(getattr(self, "max_deal_pct", 0.2)) * available)
        ...
        try:
            logging.info("[test_sample_strategy][adjust] avail=%.2f mdp=%.2f total_signals=%d cap_total=%.2f cap_per=%.2f", float(available), float(getattr(self, "max_deal_pct", 0.2)), int(total_signals), float(capital_total), float(capital_per_signal))
        except Exception:
            pass
        ...
        for s in long_pool + short_pool:
            ...
            try:
                ...
                if td in dmult and s in dmult.get(td, {}):
                    m = float(dmult[td][s])
                else:
                    m = self._get_multiplier(s)
            except Exception:
                m = self._get_multiplier(s)
            if m <= 0:
                m = 1.0
        ...
        for s in long_pool:
            ...
            margin_rate = self._get_margin_rate(s)
            if s not in symbol_info:
                continue
            p = float(symbol_info[s]["price"])
            m = float(symbol_info[s]["multiplier"])
            lots = int(target_value / max(p * m * margin_rate, 1e-6))
            if lots <= 0:
                try:
                    logging.info("[test_sample_strategy][adjust][long_zero] sym=%s tv=%.2f p=%.6f m=%.4f mr=%.4f denom=%.6f", s, target_value, p, m, margin_rate, p*m*margin_rate)
                except Exception:
                    pass
                continue
            try:
        ...
        for s in short_pool:
            ...
            if lots <= 0:
                try:
                    logging.info("[test_sample_strategy][adjust][short_zero] sym=%s tv=%.2f p=%.6f m=%.4f mr=%.4f denom=%.6f", s, target_value, p, m, margin_rate, p*m*margin_rate)
                except Exception:
                    pass
                continue
            try:
    ...
    def _need_rebalance(self, trading_day: datetime, list_bar: List[Any]) -> bool:
        ...
        try:
            ...
            pass
        try:
            if unit != "bar" and self.last_rebalance_day is not None:
                last_str = self.last_rebalance_day.strftime("%Y-%m-%d") if isinstance(self.last_rebalance_day, datetime) else str(self.last_rebalance_day)
                cur_str2 = trading_day.strftime("%Y-%m-%d")
                if last_str == cur_str2:
                    logging.info("[test_sample_strategy][rebalance_gate] skip same-day unit=%s last=%s cur=%s", unit, last_str, cur_str2)
                    return False
        except Exception:
            pass
        try:
        ...
        if idx == 0:
            try:
                logging.info("[test_sample_strategy][rebalance_gate] unit=%s idx=%d pass=%s", unit, idx, "False(first_day)")
            except Exception:
                pass
            return False
        if unit == "week":
    ...
    def _symbol_root(self, symbol: str) -> str:
        try:
            if hasattr(self.api, "get_symbol_root"):
                return self.api.get_symbol_root(symbol)
        except Exception:
            pass
        try:
                ...
                try:
                    ...
                    for r in roots:
                        ...
                        if best_sym:
                            ...
                            ed = datetime(yy, mm, last_day)
                            days_to_expire = (ed - trading_day).days
                            if days_to_expire >= 5 or days_to_expire is None:
                                if r not in new_mains or best_oi > new_mains[r][1]:
                                    new_mains[r] = (best_sym, best_oi)
                except Exception:
                    pass
